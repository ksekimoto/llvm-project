...
void RL78FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (NeedsStackRealignment) {
    ...
    const unsigned alignment = (~(MFI.getMaxAlignment() - 1)) & 0xffff;
    if (MFI.getMaxAlignment() & 0xFF00)
...
void RL78FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (NumBytesAdjusted != 0) {
    ...
    unsigned CFIIndex =
        MF.addFrameInst(MCCFIInstruction::createDefCfaOffset(nullptr, -4));
    BuildMI(MBB, MBBI, dl, TII.get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex);
  }
  if (MF.getFunction().hasFnAttribute("brk_interrupt") ||
    ...
    if (mask & (1 << 5) && mask & (1 << 4)) {
      ...
      unsigned CFIIndex = MF.addFrameInst(
          MCCFIInstruction::createDefCfaOffset(nullptr, -NumBytes - 4));
      BuildMI(MBB, MBBI, dl, TII.get(TargetOpcode::CFI_INSTRUCTION))
          .addCFIIndex(CFIIndex);
    } else {
      if (mask & (1 << 5)) {
...
int RL78FrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
