...
void RL78AsmParser::SwitchToSection(StringRef NewSectionName, unsigned Type,
  ...
  MCSection *ELFSection = getParser().getContext().getELFSection(
      NewSectionName, Type, Flags, Size, GroupName, false, (unsigned)UniqueID, (const MCSymbolELF *)Associated);
...
bool RL78AsmParser::parseDirectiveAlign() {
  ...
  if (!HasFillExpr && UseCodeAlign) {
    getStreamer().emitCodeAlignment(Alignment, &getSTI(), MaxBytesToFill);
  } else {
...
OperandMatchResultTy RL78AsmParser::tryParseRegister(unsigned &RegNo,
                                                    SMLoc &StartLoc,
                                                    SMLoc &EndLoc) {
