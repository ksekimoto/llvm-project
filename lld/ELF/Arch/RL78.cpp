...
public:
  RL78();
  void relocate(uint8_t *Loc, const Relocation &r, uint64_t Val) const override;
...
private:
  ...
  uint64_t checkAndPop(uint8_t *Loc, const Relocation &r,  char bitSize = 0, bool isSigned = false, uint64_t adj = 0) const;
...
uint64_t RL78::checkAndPop(uint8_t *Loc, const Relocation &r, char bitSize, bool isSigned, uint64_t adj) const {
  if (relocationStack.empty()) {
    error(getErrorPlace(Loc).loc + "relocation " + lld::toString(r.type) +
    ...
    if(bitSize > 0) {
      if(isSigned) {
        checkInt(Loc,t,bitSize,r);
      } else {
        checkUInt(Loc,t,bitSize,r);
      }
    }
...
void RL78::relocate(uint8_t *Loc, const Relocation &r, uint64_t Val) const {
  ...
  switch (Type) {
  case R_RL78_DIR3U:
    checkUInt(Loc, Val, 3, r);
    write3le(Loc, Val);
    break;
  case R_RL78_DIR8U:
    checkUInt(Loc, Val, 8, r);
    write8le(Loc, Val);
    break;
  case R_RL78_DIR16U:
  ...
  case R_RL78_DIR_CALLT:
    ...
    break;
  case R_RL78_ABS3U:    
    write3le(Loc, checkAndPop(Loc, r,3));
    break;
  case R_RL78_ABS8U:    
    write8le(Loc, checkAndPop(Loc, r,8));
    break;
  case R_RL78_ABS8UW:
    write8le(Loc, checkAndPop(Loc, r,8) & 0xfe);
    break;
  case R_RL78_ABS16U:
    write16le(Loc, checkAndPop(Loc, r,16));
    break;
  case R_RL78_ABS16UW:
    write16le(Loc, checkAndPop(Loc, r,16) & 0xfffe);
    break;
  case R_RL78_ABS20U:
    write24le(Loc, checkAndPop(Loc, r,20));
    break;
  case R_RL78_ABS20U_16:
    write16le(Loc, checkAndPop(Loc, r,20));
    break;
  case R_RL78_ABS20UW_16:
    write16le(Loc, checkAndPop(Loc, r,20) & 0xfffe);
    break;
  case R_RL78_ABS32U:
    write32le(Loc, checkAndPop(Loc, r,32));
    break;
  case R_RL78_ABS8S_PCREL:
    write8le(Loc, (checkAndPop(Loc, r,8,true, Val) - 1)& 0xff);
    break;
  case R_RL78_ABS16S_PCREL:
    write16le(Loc, (checkAndPop(Loc, r,16,true, Val) - 2) & 0xffff);
    break;
  case R_RL78_ABS_CALLT: {
    uint64_t savedVal = checkAndPop(Loc, r);
  ...
  case R_RL78_SYM_MIR:
    ...
    break;
  case R_RL78_OPadd: {
    uint64_t a = checkAndPop(Loc, r);
    uint64_t b = checkAndPop(Loc, r);
    push(Loc, Type, a + b);
  } break;
  case R_RL78_OPsub: {
    uint64_t a = checkAndPop(Loc, r);
  ...
  case R_RL78_OPlowW_SMIR://TODO this might be wrong PUSH (MIR(S)+ A) & 0xFFFF
    ...
    break;   
  case R_RL78_OPmir: {
    uint64_t T = checkAndPop(Loc, r);
    push(Loc, Type, checkAndConvertMirrorAddr(Loc, Type, T, true));
  } break;   
  case R_RL78_OPABSlowH:
    push(Loc, Type, (checkAndPop(Loc, r) & 0xFF00) >> 8);
    break;
  case R_RL78_OPABSlowL:
    push(Loc, Type, checkAndPop(Loc, r) & 0xFF);
    break;
  case R_RL78_OPABShighW:
    push(Loc, Type, (checkAndPop(Loc, r) & 0xFFFF0000) >> 16);
    break;
  case R_RL78_OPABSlowW:
    push(Loc, Type, checkAndPop(Loc, r) & 0xFFFF);
    break;
  default:
    error(getErrorLocation(Loc) + "unrecognized relocation " + Twine(r.type));
  }
}
