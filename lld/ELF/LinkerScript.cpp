...
void LinkerScript::addOrphanSections() {
  ...
  // Ensure unique section names generated by the #pragma address
  std::set<std::string> ATSections;
  for (InputSectionBase *isec : inputSections) {
    if (isec->name.startswith(".bss_AT") || 
      isec->name.startswith(".bssf_AT") ||
      isec->name.startswith(".const_AT") ||
      isec->name.startswith(".constf_AT")) {
      if(ATSections.find(isec->name.str()) == ATSections.end())
        ATSections.insert(isec->name.str());
      else
        for (InputSectionBase *isecOther : inputSections)
          if(isecOther->name.compare(isec->name) == 0)
            error("Address conflict: Multiple '"+toString(isecOther)+"' sections were found!");
    }
  }
            ...
            if (nameOffset) {
              ...
              if (val1.hasValue() && name.startswith(".const_AT")) {
                if (MemoryRegion *m = memoryRegions.lookup("MIRROR")) {
                  uint64_t mirrorStart = m->origin - 0xF0000;
                  uint64_t mirrorEnd = mirrorStart + m->length;
              ...
              os->addrExpr = [=] { return *val1; };
              os->lmaExpr = [=] { return *val1; };
