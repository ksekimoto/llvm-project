...
static void handleRL78InterruptAttr(Sema &S, Decl *D, const ParsedAttr &AL,
  ...
  if (AL.getNumArgs() > 0) {
    ...
    SourceLocation Loc;
    SpecsExpr->isIntegerConstantExpr(S.Context, &Loc);
    // Specs = static_cast<unsigned>(SpecsInt.getZExtValue());
    Specs = static_cast<unsigned>(Loc.getRawEncoding());
	...
	if (AL.getNumArgs() > 1) {
      ...
      SourceLocation Loc;
			VectsExpr->isIntegerConstantExpr(S.Context, &Loc);
			// ExtraDataVects.push_back(static_cast<unsigned>(VectsInt.getZExtValue()));
			ExtraDataVects.push_back(static_cast<unsigned>(Loc.getRawEncoding()));
		}
	}
  }
...
ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D, const ParsedAttr &AL,
  ...
  case ParsedAttr::AT_Address: {
    ...
    SourceLocation Loc;
    if (!addressExpr->isIntegerConstantExpr(S.Context, &Loc)) {
      S.Diag(AL.getLoc(), diag::err_attribute_argument_type)
          << AL << AANT_ArgumentIntegerConstant
    ...
    unsigned address = static_cast<unsigned>(Loc.getRawEncoding());
